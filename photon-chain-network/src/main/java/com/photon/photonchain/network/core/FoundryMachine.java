    ...
    private void blockFoundryMachine(byte[] foundryPublicKey, byte[] foundryPrivateKey) {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                if (!foundryMachineManager.foundryMachineIsStart(Hex.toHexString(foundryPublicKey))
                        || nioSocketChannelManager.getActiveNioSocketChannelCount() < Constants.FORGABLE_NODES
                        || syncBlockManager.isSyncBlock()
                    ...
                    if(syncBlockTransManager.isBlockSync()){
                        logger.info("节点回复未处理完成，稍候再出块。。。");
                        return;
                    }
                    ...
                    if (interval > Constants.BLOCK_INTERVAL && Hex.toHexString(foundryPublicKey).equals(foundryMachiner)) {
                        ...
                        builder.addAllNodeAddressList(hostList);
                        nioSocketChannelManager.write(builder.build());
                        logger.info("【新区块】广播完成，新高度={}  大小={}", block.getBlockHeight(), block.getBlockTransactions().size());
                    }
                }
            }
        }, 60000, 60000);
