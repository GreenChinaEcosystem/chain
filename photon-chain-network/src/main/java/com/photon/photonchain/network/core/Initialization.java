...
public class Initialization {
    ...
    public void init() {
        ...
        while (lastBlock == null) {
            ...
            lastBlock = blockRepository.findBlockByBlockId ( blockHeight);
            if ( lastBlock == null ) {
...
//        Transaction lastTransaction = transactionRepository.findTransactionOne(new PageRequest(0, 1, Sort.Direction.DESC, "blockHeight")).get(0);
        List<Transaction> btList=lastBlock.getBlockTransactions ( );
        if(btList.size()>0) {
            initializationManager.setLastTransaction(btList.get(btList.size()  -1));
        }
