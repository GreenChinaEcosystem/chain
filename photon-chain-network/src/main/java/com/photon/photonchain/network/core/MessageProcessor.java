...
import com.photon.photonchain.storage.encryption.cryptohash.Digest;
...
import com.photon.photonchain.storage.utils.MD5Util;
...
public class MessageProcessor {
    ...
    @Autowired
    private BlockNioSocketChannelManager blockNioSocketChannelManager;
    @Autowired
    ...
    @Autowired
    private SyncBlockTransManager syncBlockTransManager;
    ...
    public void responseProcessor(ChannelHandlerContext ctx, InesvMessage.Message msg) {
        ...
        switch (msg.getEventType()) {
            case NEW_BLOCK:
                try {
                    syncBlockTransManager.setBlockSync(true);
                    logger.info("【NEW_BLOCK,lastHeight={}】【newHeight={}】", initializationManager.getLastBlock().getBlockHeight(), MessageManager.parseBlockMessage(msg.getBlock()).getBlockHeight());
                    ...
                    if (!syncBlockManager.isSyncBlock() && !syncUnconfirmedTranManager.isSyncTransaction() && !syncTokenManager.isSyncToken() && !syncParticipantManager.isSyncParticipant()) {
                        ...
                        if (initializationManager.getBlockHeight() - newBlock.getBlockHeight() > 3) {
                            logger.error("【NEW_BLOCK--error1,lastHeight1={}   newHeight2={}】", initializationManager.getBlockHeight(), newBlock.getBlockHeight());
                            ctx.close();
                            break L;
                        }
                        if (newBlock.getBlockHeight() - initializationManager.getBlockHeight() > 3) {
                            logger.error("【NEW_BLOCK--error2,lastHeight1={}   newHeight2={}】", initializationManager.getBlockHeight(), newBlock.getBlockHeight());
                        ...
                        if (verification.verificationBlock(newBlock, msg.getParticipantCount())) {
                            ...
                            for (Transaction transaction : transactionList) {
                                try {
                                    if (transactionRepository.findByTransSignature(transaction.getTransSignature()) == null) {
                                        // 生成一个hash相关的md5串 插入
                                        transaction.setTransSignatureMd5(MD5Util.signMd5(transaction.getTransSignature()));
                                        transactionRepository.saveTransaction(transaction);
                                        settlementManager.toCalculation(transaction);
                                    }
                                    ...
                                    logger.error("校验交易异常：跳出循环 error{}", e.toString());
                                    e.printStackTrace();
                                    break L;
                                }
                            }
                            ...
                            logger.info("【新区块】回复接收完成，新高度={}", newBlock.getBlockHeight());
                            relayMessage(msg);
                        } else {
                            logger.error("【NEW_BLOCK  验证失败】");
                        }
                    } else {
                        syncBlockManager.setHasNewBlock(true);
                    }
                } finally {
                    syncBlockTransManager.setBlockSync(false);
                }
                break;
            case NEW_TRANSACTION:
                if (!syncBlockManager.isSyncBlock()
                        && !syncUnconfirmedTranManager.isSyncTransaction()
                        && !syncTokenManager.isSyncToken()
                        && !syncParticipantManager.isSyncParticipant()) {
                    ...
                    logger.info("【NEW_TRANSACTION】 something syncing.");
...
//                            unconfirmedTranRepository.saveUnconfirmedTran(unconfirmedToeknTran);
            ...
            case PUSH_MAC:
                ...
                break;
            case PUSH_MAC2:
                String mac2 = msg.getMac();
                String localhostMac2 = NetWorkUtil.getMACAddress();
                if (mac2.equals(localhostMac2)) {
                    ctx.close();
                } else {
                    blockNioSocketChannelManager.addNioSocketChannel(mac2, ctx);
                }
                break;
    ...
    public void relayMessage(InesvMessage.Message msg) {
        ...
        if (msg.getEventType() == NEW_BLOCK) {
            List<String> newHostList = blockNioSocketChannelManager.getChannelHostList();
            notifiedHostList.removeAll(newHostList);
            newHostList.addAll(notifiedHostList);
            builder.addAllNodeAddressList(newHostList);
            blockNioSocketChannelManager.writeWithOutCtxList(builder.build(), ignoreList);
        } else {
