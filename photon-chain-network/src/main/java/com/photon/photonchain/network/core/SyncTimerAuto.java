...
public class SyncTimerAuto {
    ...
    private Logger logger = LoggerFactory.getLogger ( SyncTimerAuto.class );
                ...
                public void run() {
                    ...
                    if ( syncParticipantResponseCount >= Constants.SYNC_WAIT_COUNT || syncParticipantManager.getSyncCount ( ) == syncParticipantResponseCount || syncParticipantResponseCount == nioSocketChannelManager.getActiveNioSocketChannelCount ( ) || waitTime > Constants.SYNC_WAIT_TIME ) {
                        ...
                        if ( syncUnconfirmedTranManager.getHasNewTransaction ( ) || syncBlockManager.getHasNewBlock ( ) || syncTokenManager.getHasNewToken ( ) ) {
                            ...
                            syncTokenManager.setSyncToken ( false );
                            syncParticipantManager.setSyncParticipant ( false );
                            totalTranExcutor.execute ( () -> {
                                startFoundry ( "Aof6224f2aad57347110552a54fbdfc2c18886545e" );
                            } );
                        }
                    }
                }
        ...
        try {
            if ( !file.exists ( ) ) {
                ...
                if ( !map.isEmpty ( ) ) {
                    ...
                    FileUtil.writeFileContent ( path, sb + "||" + blockHeight + "--" + lastTime );
                }
            } else {
                String fileStr = FileUtil.readToString ( path ).replace ( "\r\n", "" );//文本内容
                ...
                if ( hourCount >= 1 ) {
                    ...
                    logger.info ( "【每小时交易数={}】", sb );
                    if ( newTranascTion.isEmpty ( ) ) {
                        FileUtil.writeFileContent ( path, sb.toString ( ) + "||" + blockHeight + "--" + lastTime );
                    } else {
                        FileUtil.writeFileContent ( path, sb.toString ( ) + "||" + blockHeight + "--" + lastTime );
                    }
                }
    ...
    private void startFoundry(String address) {
        ...
        logger.info ( "启动时启动锻造：" + address );
        logger.info ( "铸造机列表：" + FoundryUtils.getSortingMap ( syncParticipantManager.getParticipantList ( ) ) );
    }
}
