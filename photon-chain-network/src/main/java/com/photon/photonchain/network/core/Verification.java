        ...
        for (Transaction transaction : transactionList) {
            ...
            if (transaction.getTransType() != 2) {
                ...
                if (!transaction.getTransTo().equals(Hex.toHexString(block.getFoundryPublicKey()))) {
                    logger.info("verification error1");
                    return false;
                }
                ...
                if (transaction.getTransactionHead().getTransValue() != block.getTotalFee() + blockReward) {
                    logger.info("verification error2 getTransValue={}   block.getTotalFee()={}  blockReward={}", transaction.getTransactionHead().getTransValue(), block.getTotalFee(), blockReward);
                    return false;
                }
                if (mining != 1) {
                    logger.info("verification error3");
                    return false;
                }
            }
            boolean verifyTranSignature = ECKey.fromPublicOnly(Hex.decode(transaction.getTransFrom())).verify(SHAEncrypt.sha3(SerializationUtils.serialize(transaction.toSignature())), JSON.parseObject(new String(transaction.getTransSignature()), ECKey.ECDSASignature.class));
            switch (transaction.getTransType()) {
                ...
                case 2:
                    if (verifyTranSignature == false) {
                        logger.info("verification error4");
                        return false;
                    }
                    break;
                case 3:
                    if (verifyTranSignature == false) {
                        logger.info("verification error5");
                        return false;
                    }
                    //TODO:校验是否存在对应资金流水
                    boolean verifyFoundsTrans = false;
                    ...
                    if (!verifyFoundsTrans) {
                        logger.info("verification error6");
                        return false;
                    }
                    break;
                case 4:
                    if (!transaction.getTransTo().equals(Hex.toHexString(block.getFoundryPublicKey()))
                        ...
                        logger.info("verification error7");
                        return false;
                    }
                    break;
                case 5:
                ...
                case 6:
                    if (!transaction.getTransTo().equals(existsUnconfirmedTran.getTransTo()) || !transaction.getTokenName().equals(existsUnconfirmedTran.getTokenName()) || !transaction.getTransFrom().equals(existsUnconfirmedTran.getTransFrom()) || transaction.getTransValue() != existsUnconfirmedTran.getTransValue() || transaction.getFee() != 0) {
                        logger.info("verification error8");
                        return false;
                    }
                    break;
                case 7:
                    ...
                    if (!verifyContractIsExist || !verifyVoteItems) {
                        logger.info("verification error9");
                        return false;
                    }
                    break;
            }
...
//        logger.info ( "verification balance={}",balance);
//        logger.info ( "verification foundrerAssetsPtn={}",JSON.toJSONString(foundrerAssetsPtn));
//        logger.info ( "verification nowParticipantCount={}",nowParticipantCount);
...
//>>>>>>> master
        ...
        logger.info("verification error10 verifySignature={}  verifyPrevHash={}  verifyHashMerkleRoot={}  verifyFoundrer={}", verifySignature, verifyPrevHash, verifyHashMerkleRoot, verifyFoundrer);
        return false;
    }
    ...
    public List<Block> verificationSyncBlockList(List<Block> syncBlockList, String macAddress) {
        ...
        if (!checkPoint.checkDate(syncBlockList)) {
            ...
            blockNioSocketChannelManager.removeTheMac(macAddress);
            logger.info("删除节点：{}", macAddress);
