...
import com.photon.photonchain.network.ehcacheManager.BlockNioSocketChannelManager;
...
import com.photon.photonchain.network.peer2.BlockPClient;
...
public class Reconnect {
    ...
    @Autowired
    private BlockPClient blockPClient;
    @Autowired
    ...
    public void init(ChannelHandlerContext ctx) {
        if (nioSocketChannelManager.getActiveNioSocketChannelCount() < Constants.FORGABLE_NODES) {
            ...
            blockNioSocketChannelManager.removeInvalidChannel();
            peerClient.init();
            blockPClient.init();
            try {
                Thread.sleep(3000);
