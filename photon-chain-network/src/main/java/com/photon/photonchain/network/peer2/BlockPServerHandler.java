...
import com.photon.photonchain.network.peer.Reconnect;
...
import static com.photon.photonchain.network.proto.EventTypeEnum.EventType.PUSH_MAC2;
...
@Component
public class BlockPServerHandler extends SimpleChannelInboundHandler<InesvMessage.Message> {

    private static Logger logger = LoggerFactory.getLogger(BlockPServerHandler.class);
    ...
    @Autowired
    private BlockNioSocketChannelManager nioSocketChannelManager;
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        logger.info(ctx.channel().remoteAddress().toString() + "block Peer通道激活");
        super.channelActive(ctx);
        String address = ctx.channel().remoteAddress().toString();
        InesvMessage.Message.Builder macBuilder = MessageManager.createMessageBuilder(RESPONSE, PUSH_MAC2);
