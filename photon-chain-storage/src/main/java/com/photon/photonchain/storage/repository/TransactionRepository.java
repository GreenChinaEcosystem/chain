...
public interface TransactionRepository extends CrudRepository<Transaction, Long> {
    ...
    @Query(value = "select transaction from Transaction transaction where transaction.transSignatureMd5=:#{#transSignatureMd5}")
    List<Transaction> findAllByTransSignatureMd5(@Param("transSignatureMd5") String transSignatureMd5);
    ...
    @Query(value = "select count(*) from Transaction transaction where transaction.transSignature=(:#{#signature})")
    int isExistBySignature(@Param("signature") byte[] transSignature);
    ...
    @Query(value = "select TRANS_TYPE,BLOCK_HEIGHT,TRANSACTION_HEAD,TOKEN_NAME,TRANS_SIGNATURE_MD5,TRANS_FROM,TRANS_TO from Transaction where lower(token_name) =lower(:#{#tokenName}) AND trans_from=:#{#account} limit :#{#start},:#{#end}", nativeQuery = true)
    List<ArrayList<Object>> findMapByTransFrom(@Param("account") String account, @Param("tokenName") String tokenName, @Param("start") long start, @Param("end") long end);

    @Query(value = "select TRANS_TYPE,BLOCK_HEIGHT,TRANSACTION_HEAD,TOKEN_NAME,TRANS_SIGNATURE_MD5,TRANS_FROM,TRANS_TO from Transaction where lower(token_name) =lower(:#{#tokenName}) AND trans_to=:#{#account} limit :#{#start},:#{#end}", nativeQuery = true)
    List<ArrayList<Object>> findMapByTransTo(@Param("account") String account, @Param("tokenName") String tokenName, @Param("start") long start, @Param("end") long end);
